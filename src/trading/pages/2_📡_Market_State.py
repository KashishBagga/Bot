...
Reads the market_state table, one row per symbol, overwritten every 5-min
candle by StructuralPaperTrader._build_market_state_record() ->
PostgresDatabase.upsert_market_state() (see src/trading/indian_trader.py).
This page only reads — it has no live broker connection of its own.
"""
import os
import sys
