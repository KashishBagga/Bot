...
unrealized_pnl_r / last_heartbeat_at are refreshed every 5-min candle by
StructuralPaperTrader._update_position() -> PostgresDatabase.update_live_heartbeat()
(see src/trading/indian_trader.py), not by this page — this page only reads.
"""
import os
import sys
...
# Candles close every 5 min; anything older than 2x that without a heartbeat
# means the trader isn't ticking (market closed, process down, etc).
STALE_AFTER = timedelta(minutes=10)
AUTO_REFRESH_SECONDS = 20

# experiment_name -> category label. Mirrors the Experiment(...) registrations
# in indian_trader.py — update here if a new experiment is registered there.
EXPERIMENT_CATEGORIES = {
    "Structural_v3.2_RVOL1.0": "Structural",
    "Structural_v3.2_RVOL0.8": "Structural",
    "EMA_Pullback_20_50_RVOL1.0": "Pullback",
    ...
    "ATR_Squeeze_RVOL1.0": "Volatility Breakout",
...
# Zero-dependency auto-refresh (no streamlit-autorefresh package needed).
...
st.caption(f"Auto-refreshes every {AUTO_REFRESH_SECONDS}s · Last loaded {now.strftime('%H:%M:%S')} IST")
...
col1.metric("Open Positions (single-leg + combo)", f"{len(positions)} + {len(combos)}")
symbols_open = sorted(set(p["symbol"] for p in positions) | set(c["symbol"] for c in combos))
col2.metric("Symbols With Open Trades", len(symbols_open))
total_unrealized = sum(p.get("unrealized_pnl_r") or 0.0 for p in positions) + sum(c.get("current_pnl_r") or 0.0 for c in combos)
col3.metric("Total Unrealized PnL", f"{total_unrealized:+.2f} R")
categories_open = sorted(set(category_of(p["experiment_name"]) for p in positions) | set(category_of(c["experiment_name"]) for c in combos))
col4.metric("Categories Represented", len(categories_open))

st.write("---")

selected_category = st.radio(
    "Filter by category", ["All"] + categories_open, horizontal=True
)
if selected_category != "All":
    positions = [p for p in positions if category_of(p["experiment_name"]) == selected_category]
    combos = [c for c in combos if category_of(c["experiment_name"]) == selected_category]
    symbols_open = sorted(set(p["symbol"] for p in positions) | set(c["symbol"] for c in combos))
...
for symbol in symbols_open:
    ...
    sym_combos = [c for c in combos if c["symbol"] == symbol]
    ...
    for p in sym_positions:
        ...
        with st.expander(title, expanded=True):
            ...
            # Chart-pattern trades carry additional candidate targets (nearest
            # opposing zone, 100% measured move, 161.8% extension) — only one
            # of these becomes the canonical `take_profit` the position manager
            # trails against, but all are informative for a "multiple targets" view.
