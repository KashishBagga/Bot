...
class MacdCrossRsiFilter:
	...
	def analyze(self, candle, index, df: pd.DataFrame, future_data: Optional[pd.DataFrame] = None):
		...
		# OPTIMIZATION: Market regime filter (low VIX proxy)
		vix_proxy = atr / price if price > 0 else 1.0
		if vix_proxy > 0.020:  # High volatility regime - avoid trading
			return None
		
		# Vol and volatility windows (tighter)
		if vol_ratio < 1.0 or atr_pct < 0.3 or atr_pct > 0.7:
			return None
		
		# MACD cross + expansion
		long_setup = macd > macd_signal and macd_hist > 0 and macd_hist > macd_hist_prev and rsi >= 45
		short_setup = macd < macd_signal and macd_hist < 0 and macd_hist < macd_hist_prev and rsi <= 55
		if not (long_setup or short_setup):
			return None
		
		# 15m trend alignment + price vs EMA21 alignment
		mtf_ok = True
		...
		if isinstance(mtf_15, pd.DataFrame) and len(mtf_15) > 50:
			...
			if not ref_slice.empty:
				...
				mtf_ok = (ema21_15 > ema50_15) if long_setup else (ema21_15 < ema50_15)
		if not mtf_ok:
			return None
		
		# Confidence
		conf = 60 + min(20, abs(macd - macd_signal) * 10) + min(10, abs(macd_hist) * 10)
		bias = 'BUY' if long_setup else 'SELL'
		
		# OPTIMIZATION: Improved R:R ratio (1.5:1 -> 2.5:1)
		stop_loss = 1.0 * atr  # Reduced from 1.3 ATR
		target = 2.5 * atr     # Increased from 2.0 ATR (2.5:1 R:R)
		slippage = price * 0.0003
		
		# Walk forward with BE after 0.6R and trail 1.0 ATR thereafter
		entry = price
		pnl = 0.0
		outcome = 'Pending'
		targets_hit = 0
		...
		for _, frow in future_data.iterrows():
			...
			# Activate BE after 0.6R
			if not breakeven_activated:
				if bias == 'BUY' and fhigh >= entry + 0.6*stop_loss:
					breakeven_activated = True
					trail_active = True
					trail_stop = entry
				elif bias == 'SELL' and flow <= entry - 0.6*stop_loss:
					breakeven_activated = True
					trail_active = True
					trail_stop = entry
			# TP at 2.5 ATR
		...
		return {
			...
			'reasoning': f'MACD cross + expansion, RSI, price>EMA21, 15m trend aligned, R:R=2.5:1, VIX={vix_proxy:.3f}'
		} 