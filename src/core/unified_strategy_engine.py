            ...
            for strategy_name, strategy_obj in self.strategies.items():
                try:
                    ...
                    if strategy_name == list(self.strategies.keys())[0]:
                        logger.info(f"🔍 Analyzing {symbol} with {len(symbol_data)} candles")
                    ...
                    if signal and isinstance(signal, dict):
                        ...
                        if (signal.get('signal') and 
                            ...
                            logger.info(f"✅ Signal generated by {strategy_name} for {symbol}: {signal_dict.get('signal')} (confidence: {signal_dict.get('confidence', 0)})")
                        else:
                            reason = signal.get('reason', 'unknown')
                            logger.info(f"⚠️ {strategy_name} generated no valid signal for {symbol}: {reason} (confidence: {confidence_score} < {self.confidence_cutoff})")
                    else:
                        logger.info(f"⚠️ {strategy_name} returned invalid analysis for {symbol}")
