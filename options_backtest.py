...
Supports both index-based approximation and historical options data
"""
...
class OptionsBacktester:
    def __init__(
        ...
        logger.info(f"🚀 Options Backtester initialized with ₹{initial_capital:,.2f} capital")
    ...
    def _estimate_option_price_and_greeks(self, symbol: str, underlying_price: float, 
                                        strike: float, option_type: str, expiry: datetime) -> Tuple[float, Dict]:
        """Estimate option price and Greeks using simplified Black-Scholes."""
        try:
            ...
            gamma = 0.02 if abs(moneyness - 1.0) < 0.1 else 0.01
            theta = -premium * 0.1 / 365  # Daily theta decay
            vega = premium * 0.5  # Vega sensitivity
    ...
    def calculate_option_position_size(self, entry_price: float, stop_loss: float, confidence: float) -> float:
        ...
        premium_risk = entry_price
        if premium_risk <= 0:
            return 0.0
        
        # Calculate lots (assuming 50 lot size for NIFTY)
        lot_size = 50
        max_lots = adjusted_risk / (premium_risk * lot_size)
        ...
        return float(max_lots * lot_size)
    ...
    def open_option_position(self, option_signal: Dict, current_price: float) -> Optional[str]:
        """Open a new option trading position."""
        try:
            ...
            position = {
                ...
                'entry_time': option_signal['timestamp'],
            ...
            logger.info(f"✅ Opened {option_signal['signal_type']} option position: {position_id}")
    ...
    def close_option_position(self, position_id: str, exit_price: float, exit_reason: str) -> Optional[Dict]:
        """Close an option trading position."""
    ...
    def check_option_position_exits(self, current_price: float, symbol: str) -> List[Dict]:
        ...
        for position_id, position in list(self.open_positions.items()):
            ...
            current_premium, greeks = self._estimate_option_price_and_greeks(
                position['contract_symbol'], current_price, position['strike'],
                position['option_type'], position['expiry']
            )
    ...
    def run_options_backtest(self, symbol: str, timeframe: str, days: int = 30) -> Dict:
        """Run options backtest."""
        logger.info(f"🚀 Starting options backtest: {symbol} {timeframe} ({days} days)")
        
        # Load data
        df = self.data_loader.load_data(symbol, timeframe, days * 24 * 60 // 5)  # Approximate candles
        ...
        for signal in all_signals:
            try:
                ...
                # Map to options
                option_signals = self.signal_mapper.map_multiple_signals([signal], signal['price'], signal['timestamp'])
                
                for option_signal in option_signals:
                    if self.should_open_option_position(option_signal):
                        position_id = self.open_option_position(option_signal, signal['price'])
                        if position_id:
                            logger.debug(f"Opened position: {position_id}")

                # Check exits
                closed_positions = self.check_option_position_exits(signal['price'], symbol)
        ...
        for position_id in list(self.open_positions.keys()):
            self.close_option_position(position_id, last_price, 'Backtest End')
        ...
        logger.info(f"✅ Options backtest completed: {total_trades} trades, {win_rate:.1f}% win rate")
...
def main():
    parser = argparse.ArgumentParser(description='Options Backtesting System')
    ...
    parser.add_argument('--historical_options', action='store_true', help='Use historical options data')
    ...
    backtester = OptionsBacktester(
        ...
        use_historical_options=args.historical_options
    )
